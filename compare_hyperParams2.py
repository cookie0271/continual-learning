...
def handle_inputs():
    ...
    parser = options.define_args(filename="compare_hyperParams2", description='Hyperparamer gridsearches.')
...
if __name__ == '__main__':
    ...
    if not utils.checkattr(args, 'no_ada'):
        print("\n\n" + "="*80)
        print(">>> 开始为 rwalk + Adaptive 执行坐标上升搜索...")
        print("="*80)

        args.weight_penalty = True
        args.offline = True
        args.importance_weighting = 'rwalk'
        args.use_adaptive = True

        # --- 步骤 1: 固定 scaling_power，寻找最优的 lambda ---
        print("\n[步骤 1/2] 正在搜索最优 lambda...")
        rwalk_adaptive_step1_results = {}
        fixed_scaling_power_rwalk = 0.7
        print(f"--- (固定 scaling_power = {fixed_scaling_power_rwalk}) ---")
        args.scaling_power = fixed_scaling_power_rwalk
        for rwalk_lambda in lambda_0_list:
            args.reg_strength = rwalk_lambda
            args.lambda_0 = rwalk_lambda # 对于rwalk, lambda_0 和 reg_strength 是一致的
            rwalk_adaptive_step1_results[rwalk_lambda] = get_result(args)
        best_rwalk_lambda = max(rwalk_adaptive_step1_results, key=rwalk_adaptive_step1_results.get)
        print(f"---> 找到最优 lambda = {best_rwalk_lambda} (准确率: {rwalk_adaptive_step1_results[best_rwalk_lambda]:.4f})")

        # --- 步骤 2: 固定最优的 lambda, 寻找最优的 scaling_power ---
        print("\n[步骤 2/2] 正在搜索最优 scaling_power...")
        rwalk_adaptive_step2_results = {}
        print(f"--- (固定 lambda = {best_rwalk_lambda}) ---")
        args.reg_strength = best_rwalk_lambda
        args.lambda_0 = best_rwalk_lambda
        for scaling_power in scaling_power_list:
            args.scaling_power = scaling_power
            rwalk_adaptive_step2_results[scaling_power] = get_result(args)
        best_scaling_power_rwalk = max(rwalk_adaptive_step2_results, key=rwalk_adaptive_step2_results.get)
        final_best_acc_rwalk = rwalk_adaptive_step2_results[best_scaling_power_rwalk]
        print(f"---> 找到最优 scaling_power = {best_scaling_power_rwalk} (准确率: {final_best_acc_rwalk:.4f})")

        print("\n" + "="*80)
        print(">>> rwalk + Adaptive 坐标上升搜索完成.")
        print(f"最终最优参数: lambda = {best_rwalk_lambda}, scaling_power = {best_scaling_power_rwalk}")
        print(f"最终最高准确率: {final_best_acc_rwalk:.4f}")
        print("="*80 + "\n")
    ...
    if not utils.checkattr(args, 'no_ada'):
        print("\n\n" + "=" * 80)
        print(">>> 开始为 SI + Adaptive 执行坐标上升搜索...")
        print("=" * 80)
        ...
        si_adaptive_step1_results = {}
        fixed_scaling_power_si = 0.7
        print(f"--- (固定 scaling_power = {fixed_scaling_power_si}) ---")
        args.scaling_power = fixed_scaling_power_si
        for lambda_0 in lambda_0_list:
            args.lambda_0 = lambda_0
            si_adaptive_step1_results[lambda_0] = get_result(args)
        best_lambda_0_si = max(si_adaptive_step1_results, key=si_adaptive_step1_results.get)
        print(
            f"---> 找到最优 lambda_0 = {best_lambda_0_si} (准确率: {si_adaptive_step1_results[best_lambda_0_si]:.4f})")

        # --- 步骤 2: 固定最优的 lambda_0, 寻找最优的 scaling_power ---
        print("\n[步骤 2/2] 正在搜索最优 scaling_power...")
        si_adaptive_step2_results = {}
        print(f"--- (固定 lambda_0 = {best_lambda_0_si}) ---")
        args.lambda_0 = best_lambda_0_si
        for scaling_power in scaling_power_list:
            args.scaling_power = scaling_power
            si_adaptive_step2_results[scaling_power] = get_result(args)
        best_scaling_power_si = max(si_adaptive_step2_results, key=si_adaptive_step2_results.get)
        final_best_acc_si = si_adaptive_step2_results[best_scaling_power_si]
        print(f"---> 找到最优 scaling_power = {best_scaling_power_si} (准确率: {final_best_acc_si:.4f})")

        print("\n" + "=" * 80)
        print(">>> SI + Adaptive 坐标上升搜索完成.")
        print(f"最终最优参数: lambda_0 = {best_lambda_0_si}, scaling_power = {best_scaling_power_si}")
        print(f"最终最高准确率: {final_best_acc_si:.4f}")
        print("=" * 80 + "\n")
    ...
    if not utils.checkattr(args, 'no_ada'):
        ...
        print(f" (固定 scaling_power = {fixed_scaling_power_rwalk})")
        print(" param-list (lambda): {}".format(list(rwalk_adaptive_step1_results.keys())))
        print("  {}".format(list(rwalk_adaptive_step1_results.values())))
        print("---> best lambda = {}  --  {}".format(best_rwalk_lambda, rwalk_adaptive_step1_results[best_rwalk_lambda]))

        print("\n\nrwalk + ADAPTIVE (步骤 2: 寻找 scaling_power)")
        print(f" (固定 lambda = {best_rwalk_lambda})")
    ...
    if not utils.checkattr(args, 'no_ada'):
        ...
        print(f" (固定 scaling_power = {fixed_scaling_power_si})")
        print(" param-list (lambda_0): {}".format(list(si_adaptive_step1_results.keys())))
        print("  {}".format(list(si_adaptive_step1_results.values())))
        print(
            "---> best lambda_0 = {}  --  {}".format(best_lambda_0_si, si_adaptive_step1_results[best_lambda_0_si]))
    ...
    if not utils.checkattr(args, 'no_ada'):
        # 图1: 寻找 lambda
        figure = my_plt.plot_lines([list(rwalk_adaptive_step1_results.values())], x_axes=list(rwalk_adaptive_step1_results.keys()),
                                   ylabel=ylabel, line_names=[f"rwalk+Adaptive (sp={fixed_scaling_power_rwalk})"],
                                   colors=["darkcyan"], title=title, x_log=True, xlabel="rwalk+Adaptive: lambda (log-scale)",
                                   with_dots=True, ylim=ylim, h_line=BASE, h_label="None")
        figure_list.append(figure)

        # 图2: 寻找 scaling_power
        figure = my_plt.plot_lines([list(rwalk_adaptive_step2_results.values())], x_axes=list(rwalk_adaptive_step2_results.keys()),
                                   ylabel=ylabel, line_names=[f"rwalk+Adaptive (λ={best_rwalk_lambda})"],
    ...
    if not utils.checkattr(args, 'no_ada'):
        # 图1: 寻找 lambda_0
        figure = my_plt.plot_lines([list(si_adaptive_step1_results.values())],
                                   x_axes=list(si_adaptive_step1_results.keys()),
                                   ylabel=ylabel, line_names=[f"SI+Adaptive (sp={fixed_scaling_power_si})"],
                                   colors=["yellowgreen"], title=title, x_log=False, xlabel="SI+Adaptive: lambda_0",
                                   with_dots=True, ylim=ylim, h_line=BASE, h_label="None")
        figure_list.append(figure)

        # 图2: 寻找 scaling_power
        figure = my_plt.plot_lines([list(si_adaptive_step2_results.values())],
                                   x_axes=list(si_adaptive_step2_results.keys()),
                                   ylabel=ylabel, line_names=[f"SI+Adaptive (λ₀={best_lambda_0_si})"],
