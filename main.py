...
def handle_inputs():
    ...
    parser.add_argument('--tasks', type=int, default=5,
                        help='Number of tasks (default: 5 for SplitMNIST)')
...
def run(args, verbose=False):
    ...
    model.use_adaptive = args.use_adaptive
    model.lambda_0 = args.lambda_0

    # 添加消融实验设置
    if hasattr(args, 'ablation_fixed_numerator'):
        model.ablation_fixed_numerator = args.ablation_fixed_numerator
        model.fixed_numerator_value = args.fixed_numerator_value
    if hasattr(args, 'ablation_fixed_denominator'):
        model.ablation_fixed_denominator = args.ablation_fixed_denominator
        model.fixed_denominator_value = args.fixed_denominator_value
    ...
    # -------------------------------------------------------------------------------------------------#
    # -------------------------------------------#
    # ----- CL-STRATEGY: ADAPTIVE PROJECTION-----#
    # -------------------------------------------#
    ...
    if isinstance(model, ContinualLearner) and checkattr(args, 'weight_penalty'):
        ...
        if not checkattr(args, 'use_adaptive'):
    ...
    if args.train:
        ...
        track_resources = hasattr(args, "track_resources") and args.track_resources
        track_time = args.time or track_resources

        # 如果命令行请求计时，则记录当前时间
        start = time.time() if track_time else None

        # 如果需要跟踪显存，重置峰值显存统计
        if track_resources and cuda:
            torch.cuda.reset_peak_memory_stats(device)
        elif track_resources and verbose and (not cuda):
            print("Requested GPU memory tracking, but CUDA is not available.")
        ...
        train_fn(
            ...
            gen_loss_cbs=generator_loss_cbs, experiment=args.experiment,
        )

        # 4. 如果请求了计时，并且 start_time 已被成功记录，则计算总时长
        if start is not None:
            ...
            # 5. 如果请求跟踪显存并且使用了 CUDA，则写入峰值显存
        if track_resources and cuda:
            peak_memory = torch.cuda.max_memory_allocated(device)
            memory_file = open("{}/memory-{}.txt".format(args.r_dir, param_stamp), 'w')
            memory_file.write('{}\n'.format(peak_memory))
            memory_file.close()
            if verbose:
                peak_mb = peak_memory / (1024 ** 2)
                print("Peak GPU memory during training = {:.1f} MB".format(peak_mb))
    ...
    # Persist per-task statistics of adaptive regularization (if available)
    if getattr(model, 'use_adaptive', False) and hasattr(model, 'lambda_task_averages'):
        lambda_path = os.path.join(args.r_dir, f"lambda-{param_stamp}.txt")
        with open(lambda_path, 'w') as lambda_file:
            for context_idx, avg_lambda in enumerate(model.lambda_task_averages, 1):
                lambda_file.write(f"{context_idx}\t{avg_lambda}\n")
        if verbose:
            print(f"Saved per-task average lambda values to {lambda_path}")

    # Persist the full training-time λ_t trajectory for DGR (if collected)
    if getattr(model, 'use_adaptive', False) and hasattr(model, 'lambda_timeline'):
        timeline_path = os.path.join(args.r_dir, f"lambda_timeline-{param_stamp}.txt")
        with open(timeline_path, 'w') as timeline_file:
            for step_idx, context_idx, lambda_value in model.lambda_timeline:
                timeline_file.write(f"{step_idx}\t{context_idx}\t{lambda_value}\n")
        if verbose:
            print(f"Saved λ_t trajectory across tasks to {timeline_path}")
    #----------------------#
    #----- EVALUATION -----#
    #----------------------#
