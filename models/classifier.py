...
class Classifier(ContinualLearner, MemoryBuffer):
    ...
    def __init__(self, image_size, image_channels, classes,
        ...
        self.classifier = fc_layer(mlp_output_size, classes, excit_buffer=True, nl='none', drop=fc_drop,
                                   phantom=phantom)
        self.initialize_regularized_params()
    ...
    def train_a_batch(self, x, y, scores=None, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, context=1,
        ...
        if self.use_adaptive and self.weight_penalty:
            '''
            # 实时更新 H_k(theta_{k-1}^*) 并调用 update_lambda
            if self.context_count > 0 and self.prev_theta_p is not None:
                if not hasattr(self, 'update_counter'):
                    self.update_counter = 0
                self.update_counter += 1

                if not hasattr(self, 'current_H_p') or self.current_H_p is None:
                    # 创建模型副本进行计算
                    model_copy = copy.deepcopy(self)
                    model_copy.load_state_dict(self.prev_theta_p, strict=False)
                    model_copy.eval()
                    # 复制输入数据以隔离
                    x_hess = x.clone().detach().requires_grad_(True)
                    y_hess = y.clone().detach()
                    self.current_H_p = model_copy.compute_hessian((x_hess, y_hess), batch_mode=True)
                    del model_copy  # 清理内存
                    torch.cuda.empty_cache()
                elif self.update_counter % 10 == 0:
                    model_copy = copy.deepcopy(self)
                    model_copy.load_state_dict(self.prev_theta_p, strict=False)
                    model_copy.eval()
                    x_hess = x.clone().detach().requires_grad_(True)
                    ...
                    for name in self.regularized_param_names:
                        if name in new_h_k and name in self.current_H_p:
                            new_current_H_p[name] = 0.1 * new_h_k[name] + 0.9 * self.current_H_p.get(name,
                                                                                                     new_h_k[name])
                    self.current_H_p = new_current_H_p
                    del model_copy
                    torch.cuda.empty_cache()
            '''
            ...
            if self.importance_weighting == 'si':
                ...
                weight_penalty_loss = self.mas_loss()
            elif self.importance_weighting == 'rwalk':
                weight_penalty_loss = self.rwalk_loss()
            self.reg_strength = self.lambda_t
            loss_total += self.reg_strength * weight_penalty_loss
        elif self.weight_penalty and not self.use_adaptive:
            if self.importance_weighting == 'si':
                weight_penalty_loss = self.surrogate_loss()
            elif self.importance_weighting == 'fisher':
                if self.fisher_kfac:
