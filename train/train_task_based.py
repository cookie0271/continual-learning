...
def train_cl(model, train_datasets, iters=2000, batch_size=32, baseline='none',
             ...
             generator=None, gen_iters=0, gen_loss_cbs=list(), experiment = None, **kwargs):
    ...
    if isinstance(model, ContinualLearner):
        ...
        elif model.importance_weighting=='mas':
            model.initialize_mas_buffers()
        elif model.importance_weighting=='rwalk':
            model.initialize_rwalk_buffers()
    ...
    for context, train_dataset in enumerate(train_datasets, 1):
        ...
        if getattr(model, 'use_adaptive', False) and len(model.lambda_logs) > 0:
            ...
            avg_lambda = float(np.mean(lambda_values)) if len(lambda_values) > 0 else float(getattr(model, 'lambda_t', 0.0))
        ...
        if context<len(train_datasets) and isinstance(model, ContinualLearner):
            ...
            ##--> MAS: calculate and update the importance weights based on sensitivity of outputs
            if model.importance_weighting=='mas' and (model.weight_penalty or model.precondition):
                model.update_mas_omega(training_dataset)
            ##--> RWalk: update the path integral and combine with Fisher Information matrix
            if model.importance_weighting=='rwalk' and (model.weight_penalty or model.precondition):
                model.update_rwalk_fisher(training_dataset)
            ##--> Adaptive Projection: update Hessian and parameter states
            if checkattr(model, 'use_adaptive'):
                model.update_after_task(training_dataset)
