...
def add_general_options(parser, main=False, comparison=False, compare_hyper=False, pretrain=False,include_seed_count=False, **kwargs):
    if main:
...
def add_eval_options(parser, main=False, comparison=False, pretrain=False, compare_replay=False, no_boundaries=False,
    ...
    if not pretrain:
        ...
        eval_params.add_argument('--track-resources', action='store_true',
                                 help="record training time and peak GPU memory usage")
    if main:
...
def add_cl_options(parser, main=False, compare_all=False, compare_replay=False, compare_hyper=False,
    ...
    if not compare_replay:
        ...
        if main and not no_boundaries:
            ...
            param_reg.add_argument('--si', action='store_true', help="select defaults for 'SI' (Zenke et al, 2017)")
            parser.add_argument('--mas', action='store_true', help="select defaults for 'MAS'")
            parser.add_argument('--rwalk', action='store_true', help="select defaults for 'RWALK'")
            ...
            param_reg.add_argument("--importance-weighting", type=str, choices=['fisher', 'si', 'owm', 'mas', 'rwalk'])
