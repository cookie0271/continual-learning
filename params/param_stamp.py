    ...
    if checkattr(args, 'weight_penalty') or checkattr(args, 'precondition'):
        ...
        if checkattr(args, 'weight_penalty'):
            if args.use_adaptive:
                param_reg_stamp += "-PRegLambda0{}".format(args.lambda_0)
                # param_reg_stamp += "-ada-L0_{}-SP_{}".format(args.lambda_0, args.scaling_power)
            else:
        ...
        if args.importance_weighting=='fisher':
            param_reg_stamp += "-FI{}{}{}{}{}{}".format(
                "kfac" if checkattr(args, 'fisher_kfac') else 'diag', ##use kfac method to approximate fisher  ##diag
                "I{}".format(args.data_size) if checkattr(args, 'fisher_init') else "",
                "N" if args.fisher_n is None else args.fisher_n, ##fisher_n sample
                "Emp" if args.fisher_labels=="true" else ("Pred" if args.fisher_labels=="pred" else (  ##fisher_lables
                    "Sam" if args.fisher_labels=="sample" else "All"
                )),
                "B{}".format(args.fisher_batch) if (hasattr(args, 'fisher_batch') and args.fisher_batch>1) else "",##fisher_batch
                # -use a separate term per task or a forgetting coefficient:
                "-offline" if checkattr(args, 'offline') else (##offline no forgetting
                    "-forg{}".format(args.gamma) if hasattr(args, 'gamma') and args.gamma < 1 else ""##-forg0.9 use forgetting params gamma(0.9) to reduce importance_weight by step
                ),
            )
      # elif args.importance_weighting=='adaptive':
        #     param_reg_stamp += "AP-fisher{}{}{}{}{}{}".format(
        #         "Diag",  # 固定使用Fisher对角线元素（取代Hessian）
        #         "I{args.init_data_size}" if args.init_data_size > 0 else "",  # 初始任务数据量
        #         "Bc{args.current_batch}" if args.current_batch != 32 else "",  # 当前任务采样批次 #rrent_batch要定义 like 'fisher_batch'
        #         "L{args.param_norm_order}" if args.param_norm_order != 2 else "",  # 参数差范数类型
        #         "γ{args.gamma_factor}",  # 标量衰减因子
        #         "-Spec" if args.use_spectral_norm else "",  # 是否计算谱范数
        #         "-Labels{args.fisher_labels}" if args.fisher_labels != "true" else ""  # Fisher标签来源
        #     )
        elif args.importance_weighting=='si':
            param_reg_stamp += "-SI{}".format(args.epsilon)
        elif args.importance_weighting=='owm':
            param_reg_stamp += "-OWM"
        elif args.importance_weighting == 'mas':
            param_reg_stamp += "-MAS{}".format(args.reg_strength)
        elif args.importance_weighting=='rwalk':
            param_reg_stamp += "-RWALK{}".format(args.reg_strength)
